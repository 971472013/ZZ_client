                    ...
                    <div id="div">
                        <table class="table table-hover">
                            ...
                            <tr v-for="i in place">
                                ...
                                <td><button class="btn-success" v-bind:name="i.name" v-on:click="check">详情</button></td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
...
<script type="text/javascript">
    ...
    <%String intro=list.get(i).getIntroduction();
    if(intro.length()>22){
    intro = intro.substring(0,20)+" ....";
    }%>
    data[<%=i%>]={img:"<%=list.get(i).getImg().substring(3)%>",name:"<%=list.get(i).getName()%>",city:"<%=list.get(i).getCity()%>",introduction:"<%=intro%>"};
    <%}%>
    new Vue({
        el:'#div',
        data:{
        ...
        methods:{
            check:function(event){
                $.post("/Manager_info",{name: encodeURI(event.target.name)},function () {
                    window.location.href="/jsp/manager_info_sure.jsp";
                },"text");
            }
        }
    });
